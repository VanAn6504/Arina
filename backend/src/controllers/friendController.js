...
export const sendFriendRequest = async (req, res) => {
  try {
    ...
    const [alreadyFriends, existingRequest] = await Promise.all([ //thuc hien 2 query cung luc
...
export const acceptFriendRequest = async (req, res) => {
  try {
    ...
    if (request.to.toString() !== userId.toString()) { //ai do co the gui loi moi kb va gui api requet tu dong chap nhan cho ban ma ko can ban chap nhan
      return res
        .status(403)
    ...
    const from = await User.findById(request.from)
      ...
      .lean();//toi uu query tra ve javsprist thay vi mongodocument
...
export const getFriendRequests = async (req, res) => {
  try {
    ...
    const [sent, received] = await Promise.all([ //dung sent va received de phan ra kq tu promise.all
